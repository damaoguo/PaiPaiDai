...
def load_2():
    ...
    a = os.path.exists("../data/amt_labels_to_use.pkl")
    b = os.path.exists("../data/clf_labels_to_use.pkl")
    c = os.path.exists("../data/sub_to_use.pkl")
    d = os.path.exists("../data/test_values_to_use.pkl")
    e = os.path.exists("../data/train_due_amt_df_to_use.pkl")
    f = os.path.exists("../data/train_values_to_use.pkl")

    if a and b and c and d and e and f:
        train_values_to_use = open('../data/train_values_to_use.pkl', 'rb')
        train_values=pickle.load(train_values_to_use)

        test_values_to_use = open('../data/test_values_to_use.pkl', 'rb')
        test_values=pickle.load(test_values_to_use)

        clf_labels_to_use = open('../data/clf_labels_to_use.pkl', 'rb')
        clf_labels=pickle.load(clf_labels_to_use)

        amt_labels_to_use = open('../data/amt_labels_to_use.pkl', 'rb')
        amt_labels=pickle.load(amt_labels_to_use)

        train_due_amt_df_to_use = open('../data/train_due_amt_df_to_use.pkl', 'rb')
        train_due_amt_df=pickle.load(train_due_amt_df_to_use)

        sub_to_use = open('../data/sub_to_use.pkl', 'rb')
        sub=pickle.load(sub_to_use)
        return train_values, test_values,clf_labels,amt_labels,train_due_amt_df,sub
    else:
        ...
        test_df = pd.read_csv('../data/test.csv',
                              parse_dates=['auditing_date', 'due_date'])  # 读入测试数据，将成交日期和应还款日期解析为日期格式
        ...
        user_info_df_1 = user_info_df.sort_values(by='info_insert_date', ascending=False).drop_duplicates(
            'user_id').reset_index(drop=True)  # 按照插入日期降序排列，去重，只保留最新的一条

        user_info_df_1['foreign_land'] = np.where(user_info_df_1['cell_province'] == user_info_df_1['id_province'], 'n',
                                                  'y')

        modifyInfoNum = user_info_df.groupby('user_id').count()['info_insert_date'].to_frame().rename(
            columns={'info_insert_date': 'modify_info_num'})
        ...
        user_tag_df_1 = user_tag_df.sort_values(by='tag_insert_date', ascending=False).drop_duplicates(
            'user_id').reset_index(drop=True)

        modifyTagListNum = user_tag_df.groupby('user_id').count()['tag_insert_date'].to_frame().rename(
            columns={'tag_insert_date': 'modify_taglist_num'})
        ...
        user_behavior_logs_1 = user_behavior_logs.groupby('user_id').count()['behavior_type'].to_frame().rename(
            columns={'behavior_type': 'behavior_num'})
        ...
        repay_log_df = repay_log_df[repay_log_df['order_id'] == 1].reset_index(
            drop=True)  # 这里采用了简化处理的方法，由于训练集和测试集均为第一期，所以这里就只保留了第一期的数据，当然，从其他期中也可以提取出很多其他的特征

        repay_log_df['repay'] = repay_log_df['repay_date'].astype('str').apply(
            lambda x: 1 if x != '2200-01-01' else 0)  # 标记是否如期还款，是，为1，逾期，为0

        repay_log_df['early_repay_days'] = (
                    repay_log_df['due_date'] - repay_log_df['repay_date']).dt.days  # 应还款日期-实际还款日期

        repay_log_df['early_repay_days'] = repay_log_df['early_repay_days'].apply(
            lambda x: x if x >= 0 else -1)  # 提前还款的就填写原始天数，否则就是逾期的，这里标上-1

        for f in ['listing_id', 'order_id', 'due_date', 'repay_date', 'repay_amt']:
            del repay_log_df[f]  # 删除标的id、标的的应还款期数序号、应还款日期、实际还款日期和实际还款金额

        group = repay_log_df.groupby('user_id',
                                     as_index=False)  # 此时该日志的字段有，user_id用户id，due_amt应还款金额，repay是否逾期，early_repay_days应还款日期-实际还款日期，然后按照user_id用户进行聚合
        ...
        del repay_log_df['repay'], repay_log_df['early_repay_days'], repay_log_df[
            'due_amt']  # 对逾期与否、应还款日与实际还款日期之差、应还款金额进行了相关统计，所以就对这三列原始值进行了删除
        ...
        cate_cols = ['gender', 'cell_province', 'id_province', 'id_city', 'foreign_land',
                     'map_age']  # 性别、手机号码归属省份、身份证归属省份、身份证归属城市

        for f in cate_cols:
            df[f] = df[f].map(dict(zip(df[f].unique(), range(df[f].nunique())))).astype(
                'int32')  # 这里相当于将对所有的值做了特征编码，通过map传入了字典，将对应的值替换为编码，并转化为int型

        df['due_amt_per_days'] = df['due_amt'] / (
                train_df['due_date'] - train_df['auditing_date']).dt.days  # 应还款金额除以(应还款日期-成交日期)

        date_cols = ['auditing_date', 'due_date', 'reg_mon', 'info_insert_date',
                     'tag_insert_date']  # 成交日期、应还款日期、用户注册年月、基础信息数据更新日期、用户画像标签更新日期
        ...
        train_values_to_use = open('../data/train_values_to_use.pkl', 'wb')
        pickle.dump(train_values, train_values_to_use, protocol=4)

        test_values_to_use = open('../data/test_values_to_use.pkl', 'wb')
        pickle.dump(test_values, test_values_to_use, protocol=4)

        clf_labels_to_use = open('../data/clf_labels_to_use.pkl', 'wb')
        pickle.dump(clf_labels, clf_labels_to_use, protocol=4)

        amt_labels_to_use = open('../data/amt_labels_to_use.pkl', 'wb')
        pickle.dump(amt_labels, amt_labels_to_use, protocol=4)

        train_due_amt_df_to_use = open('../data/train_due_amt_df_to_use.pkl', 'wb')
        pickle.dump(train_due_amt_df, train_due_amt_df_to_use, protocol=4)

        sub_to_use = open('../data/sub_to_use.pkl', 'wb')
        pickle.dump(sub, sub_to_use, protocol=4)
