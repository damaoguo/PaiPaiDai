...
for i, (trn_idx, val_idx) in enumerate(skf.split(train_values, clf_labels)):#特征数据，标签
    ...
    val_pred_prob_everyday = clf.predict_proba(val_x, num_iteration=clf.best_iteration)#预测，是一个shepe = (-1, 33)的结果，每一行的33个数代表每一类别的概率
    ...
    test_pred_prob += clf.predict_proba(test_values.values, num_iteration=clf.best_iteration) / skf.n_splits#每个测试集样本的33个类别概率
